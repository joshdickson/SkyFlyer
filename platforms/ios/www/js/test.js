...
test("Constants initialize", function() {
	...
	equal(newGameState.get('turn'), 5, "turn is 5");
	equal(newGameState.get('playerCash'), 400, "cash is 400");
	equal(newGameState.get('playerPoints'), 800, "points are 800");
	equal(newGameState.get('opponentStrength'), 85, "opp strength is 85");
});
...
test("Unit inventory initializes", function() {
	...
	equal(state.get('unitInventory').length, 4);
});
...
test("Check research project queue", function()  {
	...
	equal(state.get('buildQueueItem').get('idNumber'), 8001, "working on 8001");
	equal(state.get('buildQueueItem').get('count'), 5, "5 turns to complete");
...
test("Check units available to build", function() {
	...
	equal(state.get('availableBuildUnits').length, 4, "four units to build");
	
	// check contents
	equal(state.get('availableBuildUnits')[0].get('idNumber'), 1001, "first unit is Mustang");
	equal(state.get('availableBuildUnits')[1].get('idNumber'), 2001, "second unit is Liberator");
	equal(state.get('availableBuildUnits')[2].get('idNumber'), 3001, "third unit is Spitfire");
	equal(state.get('availableBuildUnits')[3].get('idNumber'), 4001, "fourth unit is Rocket");
});
...
test("Check research available to conduct", function() {
	...
	equal(state.getAvailableResearchProjects().length, 3, "three research projects available");
	
	// check contents
	equal(state.get('availableResearchProjects')[0].get('idNumber'), 8001, "first project is jet engines");
	equal(state.get('availableResearchProjects')[1].get('idNumber'), 8002, "second project is dogfighting");
	equal(state.get('availableResearchProjects')[2].get('idNumber'), 8008, "third project is night bombing");
});
...
test("Check complex game state formation research available", function() {
	...
	// check game state
	equal(gameState.get('availableResearchProjects').length, 5, "five projects now available");
	equal(gameState.get('availableResearchProjects')[0].get('idNumber'), 8003, "flagon factory is first project");
	equal(gameState.get('availableResearchProjects')[1].get('idNumber'), 8007, "air superiority second project");
	equal(gameState.get('availableResearchProjects')[2].get('idNumber'), 8009, "modern rocketry is third project");
	equal(gameState.get('availableResearchProjects')[3].get('idNumber'), 8004, "phantom factory is forth");
	equal(gameState.get('availableResearchProjects')[4].get('idNumber'), 8006, "stratofortress plant is fifth");
});
...
test("Check complex game state formation units available to build", function() {
	...
	equal(gameState.getAvailableBuildUnits()[0].get('idNumber'), 4001, "rocket still available");
	equal(gameState.getAvailableBuildUnits()[1].get('idNumber'), 3002, "flagon now available");
	equal(gameState.getAvailableBuildUnits()[2].get('idNumber'), 1002, "phantom now available");
	equal(gameState.getAvailableBuildUnits()[3].get('idNumber'), 2002, "stratofortress now available");
...
test("Check buy single unit", function() {
	...
	equal(game.get('gameState').get('playerUnitInventory').length, 4);
	equal(game.get('gameState').get('playerUnitInventory').at(0).get('unit').get('idNumber'), 1001);
	equal(game.get('gameState').get('playerUnitInventory').at(0).get('count'), 2);
	equal(game.get('gameState').get('playerUnitInventory').at(1).get('unit').get('idNumber'), 2001);
	equal(game.get('gameState').get('playerUnitInventory').at(1).get('count'), 0);
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('unit').get('idNumber'), 3001);
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('count'), 10);
	equal(game.get('gameState').get('playerUnitInventory').at(3).get('unit').get('idNumber'), 4001);
	equal(game.get('gameState').get('playerUnitInventory').at(3).get('count'), 5);

	// record how much cash the player has before the purchase
	var previousCash = game.get('gameState').get('playerCash');

	// now, buy a Liberator at a cost of 100 units
	var gameUnits = game.get('gameState').get('gameUnitInventory');
	game.buy(gameUnits.getByID(2001));

	equal(game.get('gameState').get('playerCash') + 100, previousCash, "show that amount was decremented for purchase")

	equal(game.get('gameState').get('playerUnitInventory').length, 4);
	equal(game.get('gameState').get('playerUnitInventory').at(0).get('unit').get('idNumber'), 1001);
	equal(game.get('gameState').get('playerUnitInventory').at(0).get('count'), 2);
	equal(game.get('gameState').get('playerUnitInventory').at(1).get('unit').get('idNumber'), 2001);
	equal(game.get('gameState').get('playerUnitInventory').at(1).get('count'), 1);
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('unit').get('idNumber'), 3001);
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('count'), 10);
	equal(game.get('gameState').get('playerUnitInventory').at(3).get('unit').get('idNumber'), 4001);
	equal(game.get('gameState').get('playerUnitInventory').at(3).get('count'), 5);
...
test("Check buy several units", function() {
	...
	var playerCashBeforeBuying = game.get('gameState').get('playerCash');

	// check the player's inventory
	equal(game.get('gameState').get('playerUnitInventory').length, 4);
	equal(game.get('gameState').get('playerUnitInventory').at(0).get('unit').get('idNumber'), 4001);
	equal(game.get('gameState').get('playerUnitInventory').at(0).get('count'), 8);
	equal(game.get('gameState').get('playerUnitInventory').at(1).get('unit').get('idNumber'), 3002);
	equal(game.get('gameState').get('playerUnitInventory').at(1).get('count'), 3);
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('unit').get('idNumber'), 2002);
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('count'), 2);
	equal(game.get('gameState').get('playerUnitInventory').at(3).get('unit').get('idNumber'), 1002);
	equal(game.get('gameState').get('playerUnitInventory').at(3).get('count'), 1);
	...
	equal(game.get('gameState').get('playerUnitInventory').length, 4);
	equal(game.get('gameState').get('playerUnitInventory').at(0).get('unit').get('idNumber'), 4001);
	equal(game.get('gameState').get('playerUnitInventory').at(0).get('count'), 10);
	equal(game.get('gameState').get('playerUnitInventory').at(1).get('unit').get('idNumber'), 3002);
	equal(game.get('gameState').get('playerUnitInventory').at(1).get('count'), 3);
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('unit').get('idNumber'), 2002);
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('count'), 3);
	equal(game.get('gameState').get('playerUnitInventory').at(3).get('unit').get('idNumber'), 1002);
	equal(game.get('gameState').get('playerUnitInventory').at(3).get('count'), 1);

	// check that the cash was spent
	equal(game.get('gameState').get('playerCash') + 400, playerCashBeforeBuying, "show that amount was decremented for purchase")
...
test('Turn increments at end of turn', function() {
	...
	equal(game.get('gameState').get('turnNumber'), 3);
	game.endTurn();
	equal(game.get('gameState').get('turnNumber'), 4);
	game.endTurn();
	equal(game.get('gameState').get('turnNumber'), 5);
...
test('Player does not make an attack, loses game', function() {
	...
	var oppStrength = game.get('gameState').get('opponentStrength');
	
	// check starting strength
	equal(game.get('gameState').get('opponentStrength'), 43);
	game.endTurn();
	equal(game.get('gameState').get('opponentStrength'), 43 + 14);
	game.endTurn();
	equal(game.get('gameState').get('opponentStrength'), 43 + 14 + 18);
	game.endTurn();
...
test('Player makes a one-unit attack attempt', function() {
	...
	game.get('gameState').get('gameUnitInventory').getByID(2002).set('defense', 1);

	// set up the attack wave
	var attackWave = new InventoryCollection();
	var attackingUnit = game.get('gameState').get('gameUnitInventory').getByID(2002);
	attackWave.add(new Inventory({'unit': attackingUnit, 'count': 1}));
	game.get('gameState').get('playerUnitInventory').findWhere({ unit: attackingUnit }).set('count', 1);
	...
	// check the player's inventory
	equal(game.get('gameState').get('playerUnitInventory').at(2).get('count'), 1);
...
test('Player makes attack attempt with two units', function() {
	...
	game.get('gameState').get('playerUnitInventory').findWhere({ unit: gameUnits.getByID(2002) }).set('count', 1);
	attackWave.add(new Inventory({'unit': gameUnits.getByID(3002), 'count': 1}));
	game.get('gameState').get('playerUnitInventory').findWhere({ unit: gameUnits.getByID(3002) }).set('count', 2);
	game.get('gameState').set('attackForce', attackWave);
